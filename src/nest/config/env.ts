...
export const envSchema = z.object({
   ...
   *
   * Значения по умолчанию совпадают с `src/config/index.js` — до переключения
   * (этап 4) обе стороны читают одни и те же переменные. Разница в моменте
   * чтения: константы того модуля вычисляются при импорте, а здесь окружение
   * разбирается при создании приложения. Это важно для тестов: они выставляют
   * `RATE_PER_SEC` в `beforeAll`, то есть уже после импорта модулей, и при
   * чтении на уровне модуля настройка терялась бы.
   */
