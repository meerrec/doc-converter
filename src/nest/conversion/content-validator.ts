 ...
 * трижды — в `api/middleware/validate.js`, в `api/routes/convert.js` и в
 * `worker/processor.js`, — причём результат `validateZip` разбирался только
 * в middleware. Два других вызова выглядели как защита, но пропускали
 * нарушения: zip-бомба, превышение числа записей и опасные имена внутри архива
 * не отклонялись. Здесь результат проверяется всегда.
 */
