...
/** Время последнего снижения числа реплик — по уровням сложности. */
const lastScaleDownAt: Record<string, number> = {};
...
async function reconcile(tier: ComplexityTier): Promise<void> {
  ...
  if (!canScaleDown(desired, current, lastScaleDownAt[tier] ?? 0, profile.cooldownMs, now)) {
    return;
  }
