 ...
 * Если файл не читается как zip или в нём нет `workbook.xml`, задача
 * классифицируется только по размеру: отказывать на этом шаге нельзя —
 * структуру уже проверил zip-гард, а решение о пригодности принимает
 * LibreOffice.
 *
...
export interface ComplexityEstimate {
  ...
  tier: ComplexityTier;
  /** Число листов книги; null — определить не удалось (нет `workbook.xml` или файл повреждён). */
  sheets: number | null;
