 ...
 *
 * Экспортирует `createServer()` в форме `{ app, server, nest }`: тестам нужен
 * слушающий сервер (его закрывают в `afterAll`), а `nest` — для корректного
 * завершения приложения.
 *
 * `app` — именно экземпляр Express (HTTP-адаптер Nest), а не Nest-приложение:
 * supertest принимает его напрямую.
 */
...
import { PinoLoggerService } from './common/nest-logger.js';
import { createLogger } from '@doc-converter/observability';
import { resolvePort, resolveTrustProxy } from './env.js';
import { closeQueues, closeRedisClient } from '@doc-converter/queue';
import {
  BODY_LIMIT_BYTES,
  applyCors,
