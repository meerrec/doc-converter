...
| Зависимости ставятся без `--prod` | В production-образ попадают инструменты сборки и тестов: `typescript` (~23 МБ), `@swc/core` с платформенным бинарником (~25 МБ), `vitest` с Vite, `supertest`, `yazl`, `concurrently`. Образ — **994 МБ**; без них было бы примерно на 100–150 МБ меньше |
