...
|---|---|
| `isolated-vm` — нативный модуль; в стадии builder поставлены `python3`, `make`, `g++` | Сборка проходит. Без этого слоя `prebuild-install` откатывался на `node-gyp rebuild` и падал с `exit status: 1` |
...
Причина — зависимость `isolated-vm`: на Node 20 она собирается, но падает с SIGSEGV при
создании изолята (проверено на 4.7.2 и 5.0.0), а совместимые версии 6.x/7.x требуют Node
22+/24+.

При этом **в конвейере конвертации `isolated-vm` не используется** — и синхронный,
и асинхронный пути работают через fork-пул. Если модуль и `worker/wasm-isolate.js`
удалить, требование Node ≥ 24 и toolchain в сборке можно снять.

Сборка образа требует toolchain (`python3`, `make`, `g++`) в стадии builder — без него
`node-gyp rebuild` для `isolated-vm` падает. Эти пакеты ставятся только в builder
и в финальный образ не переносятся.

Отдельно: `src/worker/converter.js:295 convertDocumentAsync` — заглушка, возвращающая
`{ taskId, queued: true }` и ничего не кладущая в очередь; асинхронный путь идёт через
`queue/conversionQueue.js`, а не через неё.
