...
Маршрут один для обоих входных форматов: расширение из имени файла —
подсказка, а не доказательство, и формат определяется по содержимому
контейнера. Файл без расширения принимается, если его удалось опознать;
расширение, расходящееся с содержимым, отклоняется (`magic_mismatch`).

**Тело:** `multipart/form-data`.

| Поле | Обязательное | Описание |
|---|---|---|
| `file` | да | Файл XLSX или DOCX, до 100 МиБ |
| `watermark` | нет | Текст водяного знака, до 200 символов |
| `watermarkMode` | нет | `single` (по умолчанию) или `tiled` |
| `fitToOnePage` | нет | `true` по умолчанию; действует только на книги XLSX |
...
| `exportBookmarks` | нет | `true` по умолчанию; у книги — закладки по листам, у документа — по заголовкам |
...
`inputFormat` — формат, определённый по содержимому файла: `xlsx` или `docx`.

`sheets` равен `null`, если число листов определить не удалось (повреждённый
контейнер или отсутствие `xl/workbook.xml`), а также для документов Word:
листов у них не бывает. `tier` и `queue` показывают, куда попала задача.
...
| `magic_mismatch` | 415 | Содержимое не соответствует расширению (DOCX под именем `.xlsx` и наоборот) |
| `unsupported_format` | 415 | Формат не поддерживается: не OOXML, либо расширение вне списка (`.xls`, `.doc`) |
