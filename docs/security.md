...
`worker/wasm-isolate.js` (изолят с `memoryLimit`) удалён из проекта. Причины отказа:
...
клонирования — проверено на `SubprocessConverter` и `LibreOfficeConverter`. `vm2`
не используется и не должен использоваться. Защиту по памяти обеспечивают `mem_limit`
контейнера и отдельный процесс конвертера.
