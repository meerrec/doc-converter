...
проверяется сама сборка, а не доставка. Слои кешируются в кеше GitHub
Actions — слой с LibreOffice Calc иначе качался бы из apt каждый раз.
На изменения только в документации, тестах и манифестах K8s этот workflow
не запускается.
...
tests/                      Vitest: сложность, zip-гард, скейлинг, маршруты
```
...
11. **Файлы `.xls`** (старый OLE-формат) не принимаются — сервис работает
    только с XLSX. Причина: `.xls` не является zip-контейнером, поэтому
    не проходит zip-гард, и его структура проверялась лишь восемью байтами
    сигнатуры. Через этот формат приходили и макросы Excel 4.0, которых
    в XLSX не бывает. Старые книги нужно пересохранить в XLSX.
